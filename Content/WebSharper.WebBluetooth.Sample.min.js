...
function b(){ const fW=new n("New_1");const fX=(fW.h.push(q(fW.k, "connectbluetooth", () => fW.i, () => { const f3=null;N(K(() => L(W(d()), () => M(null))), null);})),fW);const fY=(fX.h.push(new F("status", c().View)),fX);const fZ=r(fY.k, fY.h, []);const f0=new i(fZ[1], D(fZ[0]));let f1=(fY.i=f0,f0);const f2=f1.Doc;u0033("");bk.RunById("main", f2);}
function c(){ return G.statusMessage;}
function d(){ const fW=null;return new Promise((fX) => { fX(new Promise((fY) => { let fZ;fY(globalThis.navigator.bluetooth.requestDevice((fZ={},fZ.acceptAllDevices=true,fZ.optionalServices=["battery_service"],fZ)).then((f0) => { const f1=f0.name==null?"Unknown Device":f0.name;c().Set("Connected to: "+String(f1));console.log("Device Info:", f0);return f0.gatt.connect().then((f2) => { console.log("Connected to GATT Server!");return f2.getPrimaryService("battery_service").then((f3) => f3.getCharacteristic("battery_level").then((f4) => Y(W(f4.readValue())).then((f5) => { const f6=f5.getUint8(0);c().Set(u0030(c().Get())+" - Battery: "+String(f6)+"%");return Promise.resolve(null);})));});}));})["catch"]((fY) => { console.error("Bluetooth Connection Failed:", X(fY).message);c().Set("Bluetooth connection failed!");return Promise.resolve(null);}));});}
function e(fW){ throw new Error(fW);}
function f(fW){ return[fW.K, fW.V];}
function g(fW, fX){ const fY=1+fX-fW;return fY<=0?[]:bC(fY, (fZ) => fZ+fW);}
class h { Equals(fW){ return this===fW;} GetHashCode(){ return -1;} }
class i extends h { doc;allVars;anchorRoot;get Doc(){ return this.doc;} SetAnchorRoot(fW){ this.anchorRoot=fW;} constructor(fW, fX){ super();this.doc=fX;this.allVars=fW.$==0?fW.$0:e("Should not happen");this.anchorRoot=null;} }
function j(fW, fX){ l(fW, fX);return fW[fX];}
function k(fW, fX, fY){ l(fW, fX);fW[fX]=fY;}
function l(fW, fX){ if(fX<0||fX>=fW.length)e("Index was outside the bounds of the array.");}
function m(fW){ return fW.dims===2?fW.length*fW.length:fW.length;}
class n extends h { i;k;h;s;static New_1(){ return new this("New_1");} constructor(fW){ if(fW=="New_1"){ let fX;super();this.i=null;this.k=(fX=H(),String(fX));this.h=fO([]);fP(this, "s", null);} } }
class o extends h { ForTextView(){ console.warn("Content hole filled with attribute data", this.Name);return null;} AddAttribute(fW, fX){ console.warn("Var hole filled with non-Var data", this.Name);} get AsChoiceView(){ console.warn("Attribute value hole filled with non-text data", this.Name);return el("");} }
class p extends h { }
function q(fW, fX, fY, fZ){ return new I(fX, (f0) =>(f1) => { const f2=fY();f2.SetAnchorRoot(f0);return fZ({Vars:f2, Anchors:f2, Target:f0, Event:f1});});}
function r(fW, fX, fY){ const fZ=new bl("New_5");const f0=new bs("New_3");const f1=cA(fX);try { while(f1.MoveNext()){ const f2=f1.Current;const f3=f2.Name;f0.SAdd(f3);fZ.set_Item(f3, f2);} } finally { if(typeof f1=="object"&&a(f1))f1.Dispose();} return[bt(fX, bI((f4) => { const f5=f4[0];const f6=f4[1];const f7=f4[2];if(f0.Contains(f5))return null;else { const f8=f6===0?bX.GetOrAddHoleFor(fW, f5, () => { const f9=f7==null?null:u0032(f7.$0);let f_=bm.Create_1(f9==null?"":f9.$0);return new bY(f5, f_);}):f6===1?bX.GetOrAddHoleFor(fW, f5, () => { const f9=f7==null?null:u0032(f7.$0);let f_=bm.Create_1(f9==null?0:f9.$0);return new bZ(f5, f_);}):f6===2?bX.GetOrAddHoleFor(fW, f5, () => { const f9=f7==null?null:u0032(f7.$0);let f_=bm.Create_1(f9==null?false:f9.$0);return new b0(f5, f_);}):f6===3?bX.GetOrAddHoleFor(fW, f5, () => { const f9=f7==null?null:u0032(f7.$0);let f_=bm.Create_1(f9==null?-8640000000000000:f9.$0);return new b1(f5, f_);}):f6===4?bX.GetOrAddHoleFor(fW, f5, () => new b2(f5, bm.Create_1([]))):f6===5?bX.GetOrAddHoleFor(fW, f5, () => new b3(f5, bm.Create_1(u0032(globalThis.document.querySelector("[ws-dom="+f5+"]"))))):f6===6?bX.GetOrAddHoleFor(fW, f5, () => { const f9=f7==null?null:u0032(f7.$0);let f_=bm.Create_1(f9==null?[]:f9.$0);return new b4(f5, f_);}):e("Invalid value type");fZ.set_Item(f5, f8);return u0032(f8);} }, fY)), {$:0, $0:fZ}];}
function s(fW, fX){ if(fW===fX)return true;else { const fY=typeof fW;if(fY=="object"){ if(fW===null||fW===void 0||fX===null||fX===void 0||!s(typeof fX, "object"))return false;else if("Equals"in fW)return fW.Equals(fX);else if("Equals"in fX)return false;else if(fW instanceof Array&&fX instanceof Array)return t(fW, fX);else if(fW instanceof Date&&fX instanceof Date)return u(fW, fX);else { const fZ=[true];let f0;for(var f2 in fW)if(((f3) => { fZ[0]=!fW.hasOwnProperty(f3)||fX.hasOwnProperty(f3)&&s(fW[f3], fX[f3]);return!fZ[0];})(f2))break;if(fZ[0]){ let f1;for(var f3 in fX)if(((f4) => { fZ[0]=!fX.hasOwnProperty(f4)||fW.hasOwnProperty(f4);return!fZ[0];})(f3))break;} return fZ[0];} } else return fY=="function"&&("$Func"in fW?fW.$Func===fX.$Func&&fW.$Target===fX.$Target:"$Invokes"in fW&&"$Invokes"in fX&&t(fW.$Invokes, fX.$Invokes));} }
function t(fW, fX){ let fY;let fZ;if(m(fW)===m(fX)){ fY=true;fZ=0;while(fY&&fZ<m(fW)){!s(j(fW, fZ), j(fX, fZ))?fY=false:void 0;fZ=fZ+1;} return fY;} else return false;}
function u(fW, fX){ return fW.getTime()===fX.getTime();}
function v(fW){ const fX=typeof fW;return fX=="function"?0:fX=="boolean"?fW?1:0:fX=="number"?fW:fX=="string"?w(fW):fX=="object"?fW==null?0:fW instanceof Array?x(fW):y(fW):0;}
function w(fW){ let fX;if(fW===null)return 0;else { fX=5381;for(let fY=0, fZ=fW.length-1;fY<=fZ;fY++)fX=z(fX, fW[fY].charCodeAt());return fX;} }
function x(fW){ let fX=-34948909;for(let fY=0, fZ=m(fW)-1;fY<=fZ;fY++)fX=z(fX, v(j(fW, fY)));return fX;}
function y(fW){ if("GetHashCode"in fW)return fW.GetHashCode();else { const fX=[0];let fY;for(var fZ in fW)if(((f0) => { fX[0]=z(z(fX[0], w(f0)), v(fW[f0]));return false;})(fZ))break;return fX[0];} }
function z(fW, fX){ return(fW<<5)+fW+fX;}
function A(fW, fX){ if(fW===fX)return 0;else { const fY=typeof fW;switch(fY=="function"?1:fY=="boolean"?2:fY=="number"?2:fY=="string"?2:fY=="object"?3:0){ case 0:return typeof fX=="undefined"?0:-1;case 1:return e("Cannot compare function values.");case 2:return fW<fX?-1:1;case 3:if(fW===null)return -1;else if(fX===null)return 1;else if("CompareTo"in fW)return fW.CompareTo(fX);else if("CompareTo0"in fW)return fW.CompareTo0(fX);else if(fW instanceof Array&&fX instanceof Array)return B(fW, fX);else if(fW instanceof Date&&fX instanceof Date)return C(fW, fX);else { const fZ=[0];let f0;for(var f2 in fW)if(((f3) =>!fW.hasOwnProperty(f3)?false:!fX.hasOwnProperty(f3)?(fZ[0]=1,true):(fZ[0]=A(fW[f3], fX[f3]),fZ[0]!==0))(f2))break;if(fZ[0]===0){ let f1;for(var f3 in fX)if(((f4) =>!fX.hasOwnProperty(f4)?false:!fW.hasOwnProperty(f4)&&(fZ[0]=-1,true))(f3))break;} return fZ[0];} break;} } }
function B(fW, fX){ let fY;let fZ;if(m(fW)<m(fX))return -1;else if(m(fW)>m(fX))return 1;else { fY=0;fZ=0;while(fY===0&&fZ<m(fW)){ fY=A(j(fW, fZ), j(fX, fZ));fZ=fZ+1;} return fY;} }
function C(fW, fX){ return A(fW.getTime(), fX.getTime());}
function D(fW){ u0033("index");return fW?u0034("index", u0032("main"), fW):void 0;}
class E extends o { name;fillWith;get Name(){ return this.name;} get Value(){ return this.fillWith;} get AsChoiceView(){ return el(this.fillWith);} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class F extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(this.fillWith);} get AsChoiceView(){ return em(this.fillWith);} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
let G=fQ((fW) => class fX { static { G=fW(this);} static statusMessage;static { this.statusMessage=bm.Create_1("Click the button to connect Bluetooth.");} });
function H(){ return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(new RegExp("[xy]", "g"), (fW) => { const fX=Math.random()*16|0;const fY=fW=="x"?fX:fX&3|8;return fY.toString(16);});}
class I extends o { name;fillWith;get Name(){ return this.name;} get Value(){ return this.fillWith;} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
function J(fW){ const fX={};const fY=cA(fW);try { while(fY.MoveNext()){ const fZ=fY.Current;fX[fZ[0]]=fZ[1];} } finally { if(typeof fY=="object"&&a(fY))fY.Dispose();} return fX;}
function K(fW){ return(fX) => { try {(fW())(fX);} catch(fY){ fX.k(bo(fY));} };}
function L(fW, fX){ return O((fY) => { fW(bn((fZ) => { if(fZ.$==0){ const f0=fZ.$0;S().Fork(() => { try {(fX(f0))(fY);} catch(f1){ fY.k(bo(f1));} });} else S().Fork(() => { fY.k(fZ);});}, fY.ct));});}
function M(fW){ return(fX) => { fX.k(bp(fW));};}
function N(fW, fX){ const fY=(P())[0];const fZ=fX==null?fY:fX.$0;S().Fork(() => { if(!fZ.c)fW(bn((f0) => { if(f0.$==1)Q(f0.$0);}, fZ));});}
function O(fW){ return(fX) => { if(fX.ct.c)R(fX);else fW(fX);};}
function P(){ return cy.defCTS;}
function Q(fW){ console.log("WebSharper: Uncaught asynchronous exception", fW);}
function R(fW){ fW.k(bq(new c7("New", fW.ct)));}
function S(){ return cy.scheduler;}
function T(fW){ return(fX) => { const fY=[false];const fZ=(f0) => { if(fY[0])e("A continuation provided by Async.FromContinuations was invoked multiple times");else { fY[0]=true;S().Fork(f0);} };fW((f0) => { fZ(() => { fX.k(bp(f0));});}, (f0) => { fZ(() => { fX.k(bo(f0));});}, (f0) => { fZ(() => { fX.k(bq(f0));});});};}
function U(fW, fX, fY, fZ, f0){ const f1=(P())[0];const f2=f0==null?f1:f0.$0;if(!f2.c)fW(bn((f3) => { if(f3.$==1)fY(f3.$0);else if(f3.$==2)fZ(f3.$0);else fX(f3.$0);}, f2));}
function V(){ return cy.Zero;}
function W(fW){ return T((fX, fY) => { fW.then(fX, (fZ) => fY(X(fZ)));});}
function X(fW){ return fW instanceof Error?fW:new cx(fW);}
function Y(fW){ return new Promise((fX, fY) => { U(fW, fX, (fZ) => { fY(fZ);}, (fZ) => { fY(fZ);}, null);});}
function Z(fW){ let fX=[];let fY;for(var fZ in fW)fX.push(fW[fZ]);return fX;}
function u0030(fW){ return fW==null?"":fW;}
class u0031 extends h { }
...
function $(fW, fX){ let fY;const fZ=be(fW, bt(fX, bf().Values));const f0=fZ[1];const f1=fZ[0];const f2=f1.Els;return!s(f2, null)&&f2.length===1&&(j(f2, 0)instanceof Node&&(s(j(f2, 0).nodeType, Node.ELEMENT_NODE)&&(fY=j(f2, 0),true)))?c9.TreeNode(f1, f0):bk.Mk(b5(f1), f0);}
function ba(fW){ fW.removeAttribute("ws-template");const fX=fW.getAttribute("ws-replace");if(fX==null){ } else { fW.removeAttribute("ws-replace");const fY=fW.parentNode;if(s(fY, null)){ } else { const fZ=globalThis.document.createElement(fW.tagName);fZ.setAttribute("ws-replace", fX);fY.replaceChild(fZ, fW);} } const f0=globalThis.document.createElement("div");f0.appendChild(fW);return f0;}
...
class bk extends h { docNode;updates;static RunById(fW, fX){ const fY=globalThis.document.getElementById(fW);if(s(fY, null))e("invalid id: "+fW);else bk.Run(fY, fX);} static Run(fW, fX){ cG(fW, fX.docNode);bk.RunInPlace(false, fW, fX);} static get Empty(){ return bk.Mk(null, c_());} static RunInPlace(fW, fX, fY){ const fZ=cI(fX, fY.docNode);c$(dE()||dL()?dM(cJ(fW, fZ, fY.docNode)):() => { cK(fW, fZ, fY.docNode);}, fY.updates);} static Mk(fW, fX){ return new bk(fW, fX);} static TextNode(fW){ return bk.Mk(b6(globalThis.document.createTextNode(fW)), c_());} static TextView(fW){ const fX=cT();return bk.Mk(b7(fX), da((fY) => { cU(fX, fY);}, fW));} constructor(fW, fX){ super();this.docNode=fW;this.updates=fX;} }
class bl extends h { equals;hash;count;data;set_Item(fW, fX){ this.set(fW, fX);} set(fW, fX){ const fY=this.hash(fW);const fZ=this.data[fY];if(fZ==null){ this.count=this.count+1;this.data[fY]=new Array({K:fW, V:fX});} else { const f0=bJ((f1) => this.equals.apply(null, [(f(f1))[0], fW]), fZ);if(f0==null){ this.count=this.count+1;fZ.push({K:fW, V:fX});} else fZ[f0.$0]={K:fW, V:fX};} } TryGetValue(fW, fX){ const fY=this.data[this.hash(fW)];if(fY==null)return false;else { const fZ=bK((f0) => { const f1=f(f0);return this.equals.apply(null, [f1[0], fW])?u0032(f1[1]):null;}, fY);return fZ!=null&&fZ.$==1&&(fX.set(fZ.$0),true);} } ContainsKey(fW){ const fX=this.data[this.hash(fW)];return fX==null?false:bL((fY) => this.equals.apply(null, [(f(fY))[0], fW]), fX);} RemoveKey(fW){ return this.remove(fW);} get Keys(){ return new dD(this);} GetEnumerator(){ return cD(bM(Z(this.data)));} get Values(){ return new eA(this);} remove(fW){ const fX=this.hash(fW);const fY=this.data[fX];if(fY==null)return false;else { const fZ=bQ((f0) =>!this.equals.apply(null, [(f(f0))[0], fW]), fY);return m(fZ)<fY.length&&(this.count=this.count-1,this.data[fX]=fZ,true);} } Item(fW){ return this.get(fW);} get(fW){ const fX=this.data[this.hash(fW)];return fX==null?cz():bS((fY) => { const fZ=f(fY);return this.equals.apply(null, [fZ[0], fW])?u0032(fZ[1]):null;}, fX);} static New_5(){ return new this("New_5");} static New_6(fW, fX, fY){ return new this("New_6", fW, fX, fY);} constructor(fW, fX, fY, fZ){ if(fW=="New_5"){ fW="New_6";fX=[];fY=s;fZ=v;} if(fW=="New_6"){ const f0=fX;const f1=fY;const f2=fZ;super();this.equals=f1;this.hash=f2;this.count=0;this.data=[];const f3=cA(f0);try { while(f3.MoveNext()){ const f4=f3.Current;this.set(f4.K, f4.V);} } finally { if(typeof f3=="object"&&a(f3))f3.Dispose();} } } }
let bm=fQ((fW) => class fX extends h { static { bm=fW(this);} static Create_1(fY){ return new b_(false, {s:c4(fY, [])}, fY);} static { } });
...
class bs extends h { equals;hash;data;count;SAdd(fW){ return this.add(fW);} Contains(fW){ const fX=this.data[this.hash(fW)];return fX==null?false:this.arrContains(fW, fX);} add(fW){ const fX=this.hash(fW);const fY=this.data[fX];return fY==null?(this.data[fX]=[fW],this.count=this.count+1,true):this.arrContains(fW, fY)?false:(fY.push(fW),this.count=this.count+1,true);} arrContains(fW, fX){ let fY=true;let fZ=0;const f0=fX.length;while(fY&&fZ<f0)if(this.equals.apply(null, [fX[fZ], fW]))fY=false;else fZ=fZ+1;return!fY;} GetEnumerator(){ return cA(eS(this.data));} ExceptWith(fW){ const fX=cA(fW);try { while(fX.MoveNext())this.Remove(fX.Current);} finally { if(typeof fX=="object"&&a(fX))fX.Dispose();} } get Count(){ return this.count;} IntersectWith(fW){ const fX=new bs("New_4", fW, this.equals, this.hash);const fY=eS(this.data);for(let f0=0, f1=fY.length-1;f0<=f1;f0++){ const fZ=fY[f0];if(!fX.Contains(fZ))this.Remove(fZ);} } Remove(fW){ const fX=this.data[this.hash(fW)];return fX==null?false:this.arrRemove(fW, fX)&&(this.count=this.count-1,true);} CopyTo(fW, fX){ const fY=eS(this.data);for(let fZ=0, f0=fY.length-1;fZ<=f0;fZ++)k(fW, fZ+fX, fY[fZ]);} arrRemove(fW, fX){ let fY=true;let fZ=0;const f0=fX.length;while(fY&&fZ<f0)if(this.equals.apply(null, [fX[fZ], fW])){ fX.splice(fZ, 1);fY=false;} else fZ=fZ+1;return!fY;} static New_3(){ return new this("New_3");} static New_4(fW, fX, fY){ return new this("New_4", fW, fX, fY);} static New_2(fW){ return new this("New_2", fW);} constructor(fW, fX, fY, fZ){ if(fW=="New_3"){ fW="New_4";fX=[];fY=s;fZ=v;} let f0;if(fW=="New_2"){ f0=fX;fW="New_4";fX=f0;fY=s;fZ=v;} if(fW=="New_4"){ const f1=fX;const f2=fY;const f3=fZ;super();this.equals=f2;this.hash=f3;this.data=[];this.count=0;const f4=cA(f1);try { while(f4.MoveNext())this.add(f4.Current);} finally { if(typeof f4=="object"&&a(f4))f4.Dispose();} } } }
function bt(fW, fX){ return{GetEnumerator:() => { const fY=cA(fW);const fZ=[true];return new cE(fY, null, (f0) => { if(f0.s.MoveNext()){ f0.c=f0.s.Current;return true;} else { const f1=f0.s;if(!s(f1, null))f1.Dispose();f0.s=null;return fZ[0]&&(fZ[0]=false,f0.s=cA(fX),f0.s.MoveNext()?(f0.c=f0.s.Current,true):(f0.s.Dispose(),f0.s=null,false));} }, (f0) => { const f1=f0.s;if(!s(f1, null))f1.Dispose();});}};}
...
function bA(fW){ const fX=cA(fW);try { if(!fX.MoveNext())bD();let fY=fX.Current;while(fX.MoveNext()){ const fZ=fX.Current;if(A(fZ, fY)===1)fY=fZ;} return fY;} finally { if(typeof fX=="object"&&a(fX))fX.Dispose();} }
function bB(fW){ return{GetEnumerator:() => { const fX=cA(fW);function fY(fZ){ while(true){ const f0=fZ.s;if(s(f0, null)){ if(fX.MoveNext()){ fZ.s=cA(fX.Current);fZ=fZ;} else { fX.Dispose();return false;} } else if(f0.MoveNext()){ fZ.c=f0.Current;return true;} else { fZ.Dispose();fZ.s=null;fZ=fZ;} } } return new cE(null, null, fY, (fZ) => { const f0=fZ.s;if(!s(f0, null))f0.Dispose();if(!s(fX, null))fX.Dispose();});}};}
function bC(fW, fX){ return bE(fW, bF(fX));}
function bD(){ return e("The input sequence was empty.");}
function bE(fW, fX){ fW<0?dC():void 0;return{GetEnumerator:() => { const fY=[cA(fX)];return new cE(0, null, (fZ) => { fZ.s=fZ.s+1;if(fZ.s>fW)return false;else { const f0=fY[0];return s(f0, null)?dA():f0.MoveNext()?(fZ.c=f0.Current,fZ.s===fW?(f0.Dispose(),fY[0]=null):void 0,true):(f0.Dispose(),fY[0]=null,dA());} }, () => { const fZ=fY[0];if(!s(fZ, null))fZ.Dispose();});}};}
...
function bL(fW, fX){ let fY=false;let fZ=0;while(!fY&&fZ<m(fX))if(fW(fX[fZ]))fY=true;else fZ=fZ+1;return fY;}
...
function bS(fW, fX){ const fY=bK(fW, fX);return fY==null?e("KeyNotFoundException"):fY.$0;}
function bT(fW){ const fX=[];let fY=fW;while(!(fY.$==0)){ fX.push(e8(fY));fY=e9(fY);} return fX;}
function bU(fW, fX){ let fY=true;let fZ=0;while(fY&&fZ<m(fX))if(fW(fX[fZ]))fZ=fZ+1;else fY=false;return fY;}
function bV(fW, fX){ const fY=new Array(fW);for(let fZ=0, f0=fW-1;fZ<=f0;fZ++)fY[fZ]=fX;return fY;}
function bW(fW, fX){ if(fW<0)e("Negative size given.");else null;const fY=new Array(fW);for(let fZ=0, f0=fW-1;fZ<=f0;fZ++)fY[fZ]=fX(fZ);return fY;}
let bX=fQ((fW) => class fX extends h { static { bX=fW(this);} static init;id;vars;static initialized;static instances;static GetOrAddHoleFor(fY, fZ, f0){ let f1;const f2=bX.GetHolesFor(fY);const f3=(f1=null,[f2.TryGetValue(fZ, {get:() => f1, set:(f5) => { f1=f5;}}), f1]);if(f3[0])return f3[1];else { const f4=f0();f2.set_Item(fZ, f4);return f4;} } static GetHolesFor(fY){ let fZ;const f0=(fZ=null,[bX.initialized.TryGetValue(fY, {get:() => fZ, set:(f2) => { fZ=f2;}}), fZ]);if(f0[0])return f0[1];else { const f1=new bl("New_5");bX.initialized.set_Item(fY, f1);return f1;} } static { bX.initialized=new bl("New_5");bX.instances=new bl("New_5");} });
class bY extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(this.fillWith.View);} AddAttribute(fW, fX){(fW(fX))(ea(this.fillWith));} get AsChoiceView(){ return em(da(String, this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class bZ extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(da(String, this.fillWith.View));} AddAttribute(fW, fX){(fW(fX))(eb(this.fillWith));} get AsChoiceView(){ return em(da(String, this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class b0 extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(da(String, this.fillWith.View));} AddAttribute(fW, fX){(fW(fX))(ec(this.fillWith));} get AsChoiceView(){ return em(da(String, this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class b1 extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(da((fW) =>(new Date(fW)).toLocaleString(), this.fillWith.View));} AddAttribute(fW, fX){(fW(fX))(ed(this.fillWith));} get AsChoiceView(){ return em(da((fW) =>(new Date(fW)).toLocaleString(), this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class b2 extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(da(String, this.fillWith.View));} AddAttribute(fW, fX){(fW(fX))(ee(this.fillWith));} get AsChoiceView(){ return em(da(String, this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class b3 extends o { name;fillWith;get Name(){ return this.name;} get Value(){ return this.fillWith;} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
class b4 extends o { name;fillWith;get Name(){ return this.name;} ForTextView(){ return u0032(da((fW) => eo(",", fW), this.fillWith.View));} AddAttribute(fW, fX){(fW(fX))(ef(this.fillWith));} get AsChoiceView(){ return em(da(String, this.fillWith.View));} constructor(fW, fX){ super();this.name=fW;this.fillWith=fX;} }
...
class b_ extends p { isConst;current;snap;view;id;get View(){ return this.view;} Set(fW){ if(this.isConst)(((fX) => fX("WebSharper.UI: invalid attempt to change value of a Var after calling SetFinal"))((fX) => { console.log(fX);}));else { dO(this.snap);this.current=fW;this.snap={s:c4(fW, [])};} } Get(){ return this.current;} SetFinal(fW){ if(this.isConst)(((fX) => fX("WebSharper.UI: invalid attempt to change value of a Var after calling SetFinal"))((fX) => { console.log(fX);}));else { dO(this.snap);this.isConst=true;this.current=fW;this.snap={s:c5(fW)};} } UpdateMaybe(fW){ const fX=fW(this.Get());if(fX!=null&&fX.$==1)this.Set(fX.$0);} constructor(fW, fX, fY){ super();this.isConst=fW;this.current=fY;this.snap=fX;this.view=() => this.snap;this.id=c1();} }
...
function cc(fW){ const fX=bR(fW);if(fX.length==0)return{s:c5([])};else { const fY={s:c6([], [])};const fZ=[m(fX)-1];const f0=() => { if(fZ[0]===0){ const f1=bP((f2) => { const f3=f2.s;return f3!=null&&f3.$==0?f3.$0:f3!=null&&f3.$==2?f3.$0:e("value not found by View.Sequence");}, fX);if(bU((f2) => { const f3=f2.s;return f3!=null&&f3.$==0;}, fX))cq(fY, f1);else cr(fY, f1);} else fZ[0]=fZ[0]-1;};bN((f1) => { ch(f1, f0, fY);}, fX);return fY;} }
...
function cq(fW, fX){ const fY=fW.s;if(fY!=null&&fY.$==3){ fW.s=c5(fX);const fZ=fY.$0;for(let f0=0, f1=m(fZ)-1;f0<=f1;f0++)(j(fZ, f0))(fX);} else void 0;}
function cr(fW, fX){ const fY=fW.s;if(fY!=null&&fY.$==3){ fW.s=c4(fX, fY.$1);const fZ=fY.$0;for(let f0=0, f1=m(fZ)-1;f0<=f1;f0++)(j(fZ, f0))(fX);} else void 0;}
...
function cv(fW, fX){ fW.push(fX);if(fW.length%20===0){ const fY=fW.slice(0);fL(fW);for(let f0=0, f1=m(fY)-1;f0<=f1;f0++){ const fZ=j(fY, f0);if(typeof fZ=="object")(((f2) => { if(f2.s)fW.push(f2);})(fZ));else(((f2) => { fW.push(f2);})(fZ));} } else void 0;}
class cw extends h { idle;robin;Fork(fW){ this.robin.push(fW);this.idle?(this.idle=false,setTimeout(() => { this.tick();}, 0)):void 0;} tick(){ const fW=Date.now();let fX=true;while(fX)if(this.robin.length===0){ this.idle=true;fX=false;} else {(this.robin.shift())();Date.now()-fW>40?(setTimeout(() => { this.tick();}, 0),fX=false):void 0;} } constructor(){ super();this.idle=true;this.robin=[];} }
class cx extends Error { reason;constructor(fW){ super("Promise rejected");this.reason=fW;} }
let cy=fQ((fW) => class fX { static { cy=fW(this);} static GetCT;static Zero;static defCTS;static scheduler;static noneCT;static { this.noneCT=br(false, []);this.scheduler=new cw();this.defCTS=[new c8()];this.Zero=M();this.GetCT=(fY) => { fY.k(bp(fY.ct));};} });
function cz(){ throw new e1("New");}
function cA(fW){ return fW instanceof Array?cB(fW):s(typeof fW, "string")?cC(fW):fW.GetEnumerator();}
function cB(fW){ return new cE(0, null, (fX) => { const fY=fX.s;return fY<m(fW)&&(fX.c=j(fW, fY),fX.s=fY+1,true);}, void 0);}
function cC(fW){ return new cE(0, null, (fX) => { const fY=fX.s;return fY<fW.length&&(fX.c=fW[fY],fX.s=fY+1,true);}, void 0);}
function cD(fW){ return fW instanceof Array?cB(fW):s(typeof fW, "string")?cC(fW):"GetEnumerator0"in fW?fW.GetEnumerator0():fW.GetEnumerator();}
class cE extends h { s;c;n;d;e;MoveNext(){ const fW=this.n(this);this.e=fW?1:2;return fW;} get Current(){ return this.e===1?this.c:this.e===0?e("Enumeration has not started. Call MoveNext."):e("Enumeration already finished.");} Dispose(){ if(this.d)this.d(this);} constructor(fW, fX, fY, fZ){ super();this.s=fW;this.c=fX;this.n=fY;this.d=fZ;this.e=0;} }
...
function cJ(fW, fX, fY){ if(dE()){ const fZ=null;return K(() => { const f0=eD(fY);const f1=cR(fX, f0);const f2=cQ(fX, f0);return L(dF(dG(f1, cP(fX, f0))), () => L(cO(fW, fX), () => L(dF(f2), () => { fX.PreviousNodes=f0;return M(null);})));});} else return cO(fW, fX);}
...
function cO(fW, fX){ if(dL()){ const fY=(fZ) => { requestAnimationFrame(() => { cS(fW, fX.Top);fZ();});};return T((fZ, f0, f1) => fY.apply(null, [fZ, f0, f1]));} else { cS(fW, fX.Top);return M(null);} }
...
class c8 extends h { init;c;pending;r;constructor(){ super();this.c=false;this.pending=null;this.r=[];this.init=1;} }
class c9 extends bk { docNode_1;updates_1;elt;rvUpdates;static TreeNode(fW, fX){ const fY=ez.Create(fX);const fZ=bP((f2) => dQ(f2[1]), fW.Attrs);let f0=ew(c_(), de, fZ);let f1=de(f0, fY.v);return new c9(b5(fW), f1, j(fW.Els, 0), fY);} constructor(fW, fX, fY, fZ){ super(fW, fX);this.docNode_1=fW;this.updates_1=fX;this.elt=fY;this.rvUpdates=fZ;} }
function c_(fW){ const fX={s:c5(fW)};return() => fX;}
function c$(fW, fX){ function fY(){ b$(fX(), fW, () => { S().Fork(fY);});} S().Fork(fY);}
...
function dg(fW){ let fX=null;let fY=0;const fZ=fW.textContent;const f0=new RegExp(bj(), "g");while(fX=f0.exec(fZ),fX!==null){ fW.parentNode.insertBefore(globalThis.document.createTextNode(do_1(fZ, u0032(fY), u0032(f0.lastIndex-j(fX, 0).length-1))), fW);fY=f0.lastIndex;const f1=globalThis.document.createElement("span");f1.setAttribute("ws-replace", j(fX, 1).toLowerCase());fW.parentNode.insertBefore(f1, fW);} f0.lastIndex=0;fW.textContent=do_1(fZ, u0032(fY), null);}
function dh(fW){ console.warn("Instantiating non-loaded template", fW);}
function di(fW, fX, fY){ const fZ=fW.querySelector("[ws-replace]");return s(fZ, null)?(console.warn("Filling non-existent text hole", fY),null):(fZ.parentNode.replaceChild(globalThis.document.createTextNode(fX), fZ),u0032(fZ.getAttribute("ws-replace")));}
function dj(fW, fX){ const fY=(fZ) => { bg(fW, "["+fZ+"]", (f0) => { if(!fX.Contains(f0.getAttribute(fZ)))f0.removeAttribute(fZ);});};fY("ws-attr");fY("ws-onafterrender");fY("ws-var");bg(fW, "[ws-hole]", (fZ) => { if(!fX.Contains(fZ.getAttribute("ws-hole"))){ fZ.removeAttribute("ws-hole");while(fZ.hasChildNodes())fZ.removeChild(fZ.lastChild);} });bg(fW, "[ws-replace]", (fZ) => { if(!fX.Contains(fZ.getAttribute("ws-replace")))fZ.parentNode.removeChild(fZ);});bg(fW, "[ws-on]", (fZ) => { fZ.setAttribute("ws-on", eo(" ", bQ((f0) => fX.Contains(j(ep(f0, [":"], 1), 1)), ep(fZ.getAttribute("ws-on"), [" "], 1))));});bg(fW, "[ws-attr-holes]", (fZ) => { const f0=ep(fZ.getAttribute("ws-attr-holes"), [" "], 1);for(let f4=0, f5=f0.length-1;f4<=f5;f4++){ const f1=j(f0, f4);let f2=new RegExp(bj(), "g");let f3=fZ.getAttribute(f1).replace(f2, (f6, f7) => fX.Contains(f7)?f6:"");fZ.setAttribute(f1, f3);} });}
function dk(fW, fX){ dn(fX);const fY=fX.nodeName.toLowerCase();const fZ=fW.querySelector("[ws-attr="+fY+"]");if(s(fZ, null))console.warn("Filling non-existent attr hole", fY);else { fZ.removeAttribute("ws-attr");for(let f1=0, f2=fX.attributes.length-1;f1<=f2;f1++){ const f0=fX.attributes.item(f1);if(f0.name=="class"&&fZ.hasAttribute("class"))fZ.setAttribute("class", fZ.getAttribute("class")+" "+f0.nodeValue);else fZ.setAttribute(f0.name, f0.nodeValue);} } }
function dl(fW, fX){ const fY=(fZ) => { bg(fW, "["+fZ+"]", (f0) => { let f1;const f2=(f1=null,[fX.TryGetValue(f0.getAttribute(fZ).toLowerCase(), {get:() => f1, set:(f3) => { f1=f3;}}), f1]);if(f2[0])f0.setAttribute(fZ, f2[1]);});};fY("ws-hole");fY("ws-replace");fY("ws-attr");fY("ws-onafterrender");fY("ws-var");bg(fW, "[ws-on]", (fZ) => { fZ.setAttribute("ws-on", eo(" ", bP((f0) => { let f1;const f2=ep(f0, [":"], 1);const f3=(f1=null,[fX.TryGetValue(j(f2, 1), {get:() => f1, set:(f4) => { f1=f4;}}), f1]);return f3[0]?j(f2, 0)+":"+f3[1]:f0;}, ep(fZ.getAttribute("ws-on"), [" "], 1))));});bg(fW, "[ws-attr-holes]", (fZ) => { const f0=ep(fZ.getAttribute("ws-attr-holes"), [" "], 1);for(let f1=0, f2=f0.length-1;f1<=f2;f1++)((() => { const f3=j(f0, f1);return fZ.setAttribute(f3, bv((f4, f5) => { const f6=f(f5);return f4.replace(new RegExp("\\${"+f6[0]+"}", "ig"), "${"+f6[1]+"}");}, fZ.getAttribute(f3), fX));})());});}
...
function dy(fW, fX, fY){ let fZ;if(fY.parentNode===fW){ const f0=fY.nextSibling;let f1=s(f0, null)?null:f0;fZ=fX===f1;} else fZ=false;if(!fZ)fW.insertBefore(fY, fX);}
function dz(fW, fX){ if(fX.parentNode===fW)fW.removeChild(fX);}
function dA(){ return e("The input sequence has an insufficient number of elements.");}
function dB(fW, fX){ let fY=true;let fZ=0;while(fY&&fZ<m(fX))if(s(fX[fZ], fW))fY=false;else fZ=fZ+1;return!fY;}
function dC(){ return e("The input must be non-negative.");}
class dD extends h { d;GetEnumerator(){ return cA(bx((fW) => fW.K, this.d));} constructor(fW){ super();this.d=fW;} }
function dE(){ return eJ();}
function dF(fW){ const fX=null;return K(() => L(dH(() => { }, eK(fW)), () => { eL(fW);return M(null);}));}
function dG(fW, fX){ return dI(eW(fW.$0, fX.$0));}
function dH(fW, fX){ const fY=fX.Duration;if(fY===0)return V();else { const fZ=(f0) => { function f1(f2){ return(f3) => { const f4=f3-f2;fX.Compute(f4);fW();return f4<=fY?void requestAnimationFrame((f5) => {(f1(f2))(f5);}):f0();};} requestAnimationFrame((f2) => {(f1(f2))(f2);});};return T((f0, f1, f2) => fZ.apply(null, [f0, f1, f2]));} }
...
function dM(fW){ const fX=[0];function fY(){ const fZ=null;return K(() => L(fW, () => { const f0=fX[0];return s(f0, 1)?(fX[0]=0,V()):s(f0, 2)?(fX[0]=1,fY()):V();}));} return() => { const fZ=fX[0];if(s(fZ, 0)){ fX[0]=1;N(fY(), null);} else s(fZ, 1)?fX[0]=2:void 0;};}
function dN(fW, fX, fY, fZ){ const f0={DynElem:fW, DynFlags:fX, DynNodes:fY};fP(f0, "OnAfterRender", fZ);return f0;}
function dO(fW){ let fX;const fY=fW.s;if(fY==null||(fY!=null&&fY.$==2?(fX=fY.$1,false):fY!=null&&fY.$==3?(fX=fY.$1,false):true))void 0;else { fW.s=null;for(let f0=0, f1=m(fX)-1;f0<=f1;f0++){ const fZ=j(fX, f0);if(typeof fZ=="object")(((f2) => { dO(f2);})(fZ));else fZ();} } }
...
class d8 extends o { name;fillWith;get Value(){ return this.fillWith;} get Name(){ return this.name;} }
...
class ek extends o { name;fillWith;get Value(){ return this.fillWith;} get Name(){ return this.name;} }
...
function ew(fW, fX, fY){ const fZ=m(fY);function f0(f1){ return(f2) => { let f3;switch(f2<=0?0:f2===1?f1>=0&&f1<fZ?1:(f3=f2,2):(f3=f2,2)){ case 0:return fW;case 1:return j(fY, f1);case 2:const f4=f2/2>>0;return fX((f0(f1))(f4), (f0(f1+f4))(f2-f4));} };} return(f0(0))(fZ);}
function ex(fW, fX, fY, fZ){ const f0=m(fZ);function f1(f2){ return(f3) => { let f4;switch(f3<=0?0:f3===1?f2>=0&&f2<f0?1:(f4=f3,2):(f4=f3,2)){ case 0:return fX;case 1:return fW(j(fZ, f2));case 2:const f5=f3/2>>0;return fY((f1(f2))(f5), (f1(f2+f5))(f3-f5));} };} return(f1(0))(f0);}
...
class eA extends h { d;GetEnumerator(){ return cA(bx((fW) => fW.V, this.d));} constructor(fW){ super();this.d=fW;} }
...
function eM(fW){ const fX=ey(fW);const fY=m(fX);if(fY===0)return eO();else if(fY===1)return j(fX, 0);else { const fZ=bA(bx((f1) => f1.Duration, fX));const f0=bP((f1) => eQ(fZ, f1), fX);return eP(fZ, (f1) => { bN((f2) => { f2.Compute(f1);}, f0);});} }
...
function e_(){ return e("The input list was empty.");}
function e$(fW, fX, fY){ let fZ;fZ=null;return[(f0) => { const f1=() => { fY.UpdateMaybe((f2) => { let f3;fZ=fW(f0);return fZ!=null&&fZ.$==1&&(!s(fZ.$0, f2)&&(f3=[fZ, fZ.$0],true))?f3[0]:null;});};f0.addEventListener("change", f1);f0.addEventListener("input", f1);f0.addEventListener("keypress", f1);}, (f0) => { const f1=fX(f0);return(f2) => f2==null?null:f1(f2.$0);}, da((f0) => { let f1;return fZ!=null&&fZ.$==1&&(s(fZ.$0, f0)&&(f1=fZ.$0,true))?null:u0032(f0);}, fY.View)];}
...
function fg(fW, fX, fY){ let fZ;fZ=null;return[(f0) => { f0.addEventListener("change", () => { fY.UpdateMaybe((f1) => { let f2;fZ=fW(f0);return fZ!=null&&fZ.$==1&&(fZ.$0!==f1&&(f2=[fZ, fZ.$0],true))?f2[0]:null;});});}, (f0) => { const f1=fX(f0);return(f2) => f2==null?null:f1(f2.$0);}, da((f0) => { let f1;return fZ!=null&&fZ.$==1&&(s(fZ.$0, f0)&&(f1=fZ.$0,true))?null:u0032(f0);}, fY.View)];}
...
function fL(fW){ fW.splice(0, m(fW));}
